...
class SensorReading(object):
    def __init__(self):
        self.id=choice(["690001001","cam6900001","690000996"])
        self.cycle=randint(0,10)
...
host = server+"/smavoo/api/device/status"

def foo():
    sr = SensorReading()
    data_json = sr.toJSON()
    response = requests.post(host, data=data_json, auth=('SuperUser','sys'), headers={'content-type': 'application/json'})
    print(response)
    print(response.text)
    threading.Timer(4, foo).start()
